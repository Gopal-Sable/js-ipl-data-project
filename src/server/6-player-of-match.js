...
const results= matches.reduce((manOfMatch, match)=>{
    ...
    if (!manOfMatch[match.season][match.player_of_match]) {
        manOfMatch[match.season][match.player_of_match]=0;
    }
...
for (const key in results) {
  ...
  Object.entries(results[key]).sort((a,b)=> b[1]-a[1]).slice(0,1).map(data=>{
    man[key]=data[0];
  });
}
